...
'''
Blackjack Game simulation based on the PlayingCard
and BlackJack classes. Provides the user interface
and game flow for a Blackjack Simulation.
This file is the game entry point
'''
...
class Game:
    ...
    def __init__(self):
        self.table = BlackJackEngine()
        # self.table.get_free_seats() #<=== get an array of
        # free seat indecies to use in .add_player()
    ...
    def payout_blackacks(self):
        naturals = list(
            filter(lambda b: b.hand.is_blackjack, self.table.get_bets()))
        for bet in naturals:
            bet.hand.fold_hand()
    ...
    def betting_loop(self):
        for bet in self.table.bets:
            name = bet.seat.player.name
            house = self.table.house_name
            folded = bet.hand.folded
            if name != house and not folded:
                self.display(bet)

                # STEP 1:   In case of a previous "split" this hand
                #           COULD have only 1 card, so let's fix that
                if len(bet.hand.cards) == 1:
                    self.table.hit(bet.hand)
                    self.display(bet)

                # STEP 2:   Now let's run the main loop to see what action
                #           the player wants to take on this hand
                cont = True
                while cont:
    ...
    def insurance_input(self, bet):
            nm = bet.seat.player.name
            input(f"{nm}: would you like insurance? (y/n): ")[0].lower() == 'y'
