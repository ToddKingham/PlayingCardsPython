...
'''
    Provides a virtual playing card deck (or decks) specifically
    designed to retain the physics and properties of a physical deck
'''
...
class PlayingCards():
    ...
    def __init__(self, params=params, deck=None):
        self.CARDS = {
            ...
            "discard": [],
            "table": [],
            # maybe we will use this when "laying down" in games such as rummy or even poker games like texas hold 'em
            "hands": [],
            "dealer": 0,
            "player": 1
        }
            ...
            for _ in range(self.params['packs']):
                for cardset in self.params['cards']:
                    for s in cardset['suit']:
                        ...
                        if len(suit) == 2 and suit[1] == 'Desc':
                            indecies.reverse()
                        self.CARDS['stock'] += list(
                            map(lambda i: Card(i, suit[0]), indecies)
                        )
        else:
            self.CARDS = deck
    ...
    def shuffle(self, typ=['wash'], pile='stock'):
        ...
        def box(p):
            ...
            for _ in range(3):
                b = Helpers().splice(
                    p, 0, quarter + Helpers().plusOrMinus(4)
                ) + b
            p[len(p)::] = b
    ...
    def draw(self, idx, count=1, pile='stock', face_up=False):
        ...
        Helpers().move(
            pile, self.CARDS['hands'][idx], 0, count, face_up=face_up)
